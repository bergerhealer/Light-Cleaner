...
public final class WorldEditHandlerV2 implements Handler {
    ...
    public String getEnableMessage() {
        return "Added support for automatic light cleaning when WorldEdit (>= v7.0.0) operations are performed!";
    }

    @Override
    public String getDisableMessage() {
        return "WorldEdit (>= v7.0.0) was disabled, automatic light cleaning is disabled again";
    }

    @Override
    public HandlerInstance enable(HandlerOps ops) throws Exception, Error {
        return new WEV2HandlerInstance(ops);
    }

    private static final class WEV2HandlerInstance extends HandlerInstance {
        private final EventBus eventBus;

        public WEV2HandlerInstance(HandlerOps ops) {
            super(ops);
